...
public class GLDisplay {
		...
		try {
			...
			if (fullscreen) {
				...
				usedDevice.setDisplayMode(findDisplayMode(usedDevice
						.getDisplayModes(), width, height, usedDevice
						.getDisplayMode().getBitDepth(), usedDevice
	...
	public void registerMouseEventForHelp(int id, int modifiers,
			String description) {
		helpOverlayGLEventListener.registerMouseEvent(id, modifiers,
				description);
	}
	...
	private class MyKeyAdapter extends KeyAdapter {
		public MyKeyAdapter() {
			registerKeyStrokeForHelp(KeyStroke.getKeyStroke(KeyEvent.VK_F1, 0),
					"Show/hide help message");
			registerKeyStrokeForHelp(
					KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), "Quit demo");
		}
			...
			SwingUtilities.invokeLater(new Runnable() {
				public void run() {
					...
					JOptionPane.showMessageDialog(frame,
							stringWriter.toString(), "Exception occurred",
	...
	private static class MyHelpOverlayGLEventListener implements
			GLEventListener {
		...
		public void displayChanged(GLAutoDrawable glDrawable, boolean b,
				boolean b1) {
			for (int i = 0; i < eventListeners.size(); i++) {

				// ( (GLEventListener) eventListeners.get( i ) ).displayChanged(
				// glDrawable, b, b1 );
			}
		}
		...
		public void reshape(GLAutoDrawable glDrawable, int i0, int i1, int i2,
				int i3) {
			for (int i = 0; i < eventListeners.size(); i++) {
				((GLEventListener) eventListeners.get(i)).reshape(glDrawable,
						i0, i1, i2, i3);
			}
		}
