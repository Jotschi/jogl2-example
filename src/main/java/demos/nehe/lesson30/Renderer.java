...
class Renderer implements GLEventListener {
	...
	private Tuple3d veloc = new Tuple3d(.5, -.1, .5); // initial velocity of
														// balls
	private Tuple3d accel = new Tuple3d(0, -.05, 0); // acceleration ie. gravity
														// of balls
	private Tuple3d dir = new Tuple3d(0, 0, -10); // initial direction of camera
	private Tuple3d cameraPosition = new Tuple3d(0, -50, 1000); // initial
																// position of
																// cameraT
	...
	private boolean cameraAttachedToBall = false; // hook camera on ball, and
													// sound on/off
	private boolean soundsEnabled = true; // hook camera on ball, and sound
											// on/off

	private Plane pl1, pl2, pl3, pl4, pl5; // the 5 planes of the room
	private Cylinder cyl1, cyl2, cyl3; // the 2 cylinders of the room
	private Explosion[] explosions = new Explosion[20]; // holds max 20
														// explosions at once
	...
	public void init(GLAutoDrawable drawable) {
		...
		gl.glMaterialfv(GL.GL_FRONT, GL2.GL_SHININESS, df, 0);
		...
		try {
			audioSample = new AudioSample(
					ResourceRetriever
							.getResourceAsStream("demos/data/samples/Explode.wav"));
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void loadGLTextures(GL gl) {
		 ...
		 * the data itself.
		 */
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, 3, image2.getWidth(),
		 ...
		 * the data itself.
		 */
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, 3, image3.getWidth(),
		 ...
		 * the data itself.
		 */
		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, 3, image4.getWidth(),
	...
	public void display(GLAutoDrawable drawable) {
		...
		if (cameraAttachedToBall)
			glu.gluLookAt(arrayPos[0].x + 250, arrayPos[0].y + 250,
					arrayPos[0].z, arrayPos[0].x + arrayVel[0].x, arrayPos[0].y
							+ arrayVel[0].y, arrayPos[0].z + arrayVel[0].z, 0,
					1, 0);
		else
			glu.gluLookAt(cameraPosition.x, cameraPosition.y, cameraPosition.z,
					cameraPosition.x + dir.x, cameraPosition.y + dir.y,
					cameraPosition.z + dir.z, 0, 1.0, 0.0);
		...
		for (i = 0; i < 20; i++) {
			if (explosions[i].alpha >= 0) {
				...
				gl.glScalef(explosions[i].scale, explosions[i].scale,
						explosions[i].scale);
				gl.glTranslatef((float) explosions[i].position.x
						/ explosions[i].scale, (float) explosions[i].position.y
						/ explosions[i].scale, (float) explosions[i].position.z
						/ explosions[i].scale);
				gl.glCallList(dlist);
				gl.glPopMatrix();
			}
	...
	/**
	 * Main loop of the simulation. Moves, finds the collisions and responses of
	 * the objects in the current time step.
	 */
	private void idle() {
	 ...
	 */
	private boolean TestIntersionPlane(Plane plane, Tuple3d position,
	...
	/**
	 * Find if any of the current balls intersect with each other in the current
	 * timestep.
	 * 
	 * @return the index of the 2 itersecting balls, the point and time of
	 *         intersection
	 */
	private int findBallCol(Tuple3d point, double[] TimePoint, double[] Time2,
	...
	private static class TRay {
		...
		public double dist(Tuple3d point) {
			double lambda = v.dot(new Tuple3d(point.x - p.x, point.y - p.y,
					point.z - p.z));
