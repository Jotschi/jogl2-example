 ...
 * 
 * Author: Roman Podobedov Email: romka@ut.ee WEB: http://romka.demonews.com
 * Release Date: 18 May 2001
 * 
 * Copyright (C) 2000-2001, Romka Graphics This library is freely distributable
 * without any license or permissions for non-commercial usage. You can use this
 * library in any non-commercial program. In each program, where you use this
 * library you should keep this header (author name and coordinates)! For
 * commercial usage, please contact me: romka@ut.ee
 * 
...
public class GLF {
	...
	private float SymbolDist = 0.2f; /*
									 * Distance between symbols (Variable
									 * constant)
									 */
	// Symbol Depth in 3D space (Variable constant)
	private float SymbolDepth = 0.2f; 
	...
	// private float RotateAngleB = 0.0f; /* Rotate angle for string (bitmap
	// font) */
	...
	private int conWidth, conHeight; 
	//Console current X and Y
	private int conx = 0, cony = 0;  
	//Console data
	private StringBuffer[] conData; 
	// Console font 
	private int conFont; 
	// Console cursor Enabled|Disabled
	private char conCursor = GLF_NO;  
	// Console cursor blink rate
	private int conCursorBlink;  
	// Console cursor blink counter
	private int conCursorCount; 
	// Console Cursor mode (on/off screen)
	private char conCursorMode = GLF_NO; 
	...
	// All fonts
	private int[] bmf_texture = new int[MAX_FONTS]; 
	// All fonts (masked)
	private int[] bmf_mask = new int[MAX_FONTS]; 
	// 1 - if font is used, 0 - otherwise
	private boolean[] bmf_in_use = new boolean[MAX_FONTS]; 
	private int[] list_base = new int[MAX_FONTS];
	private float[] m_max_height = new float[MAX_FONTS]; /*
														 * Maximal height of
														 * each font
														 */
	private widths[] m_widths = new widths[MAX_FONTS]; /*
														 * Width of each symbols
														 * in each font
														 */
	...
	private int readFont(String font_name, GLFFont glff) {
		...
		try {
			...
			InputStream stream = ClassLoader
					.getSystemResourceAsStream(font_name);
			// If not found in jar, then load from disk
			if (stream == null) {
				inputStream = new LittleEndianInputStream(
						new BufferedInputStream(new FileInputStream(font_name)));
			} else {
				inputStream = new LittleEndianInputStream(
			...
			glff.sym_total = inputStream.readUnsignedByte(); /*
															 * Read total
															 * symbols in font
															 */
			...
			for (int i = 0; i < glff.sym_total; i++) {
				...
				for (int j = 0; j < fcets; j++) {
					glff.symbols[code].fdata[j * 3] = inputStream
							.readUnsignedByte();
					glff.symbols[code].fdata[j * 3 + 1] = inputStream
							.readUnsignedByte();
					glff.symbols[code].fdata[j * 3 + 2] = inputStream
							.readUnsignedByte();
				}
				for (int j = 0; j < lns; j++) {
					glff.symbols[code].ldata[j] = inputStream
							.readUnsignedByte();
				}
	...
	/*
	 * | Function loads font to memory from file | Return value: GLF_ERROR - if
	 * error | >=0 - returned font descriptor (load success)
	 */
	...
	/*
	 * | Unload current font from memory | Return value: GLF_OK - if all OK |
	 * GLF_ERROR - if error
	 */
	...
	private void drawString(GL2 gl, String s, DrawSymbol funct) {
		...
		if (m_string_center) {
			...
			for (i = 0; i < s.length(); i++) {
				if ((fonts[curfont].symbols[s.charAt(i)] == null)
						|| (s.charAt(i) == ' ')) {
					...
					if (s.charAt(i + 1) == ' ') {
						...
						if (m_direction == GLF_LEFT || m_direction == GLF_RIGHT) {
							sda = Math
									.abs(fonts[curfont].symbols[s.charAt(i)].rightx);
							sdb = Math.abs(fonts[curfont].symbols[s
									.charAt(i + 1)].leftx);
							...
							sda = Math
									.abs(fonts[curfont].symbols[s.charAt(i)].topy);
							sdb = Math
									.abs(fonts[curfont].symbols[s.charAt(i)].bottomy);
		...
		if (m_string_center) {
			...
			switch (m_direction) {
			case GLF_LEFT:
				gl.glTranslatef(-(1 - Math.abs(fonts[curfont].symbols[s
						.charAt(0)].leftx)), 0, 0);
				break;
			case GLF_RIGHT:
				gl.glTranslatef((1 - Math.abs(fonts[curfont].symbols[s
			...
			case GLF_UP:
				gl.glTranslatef(
						0,
						(1 - Math.abs(fonts[curfont].symbols[s.charAt(0)].topy)),
						0);
				break;
			case GLF_DOWN:
				gl.glTranslatef(0, -(1 - Math.abs(fonts[curfont].symbols[s
						.charAt(0)].bottomy)), 0);
				break;
			}
		}
			...
			if ((fonts[curfont].symbols[s.charAt(i)] == null)
				...
				if (i < (s.length() - 1)) {
					if (s.charAt(i + 1) == ' ') {
						...
						if (m_direction == GLF_LEFT || m_direction == GLF_RIGHT) {
							if (m_direction == GLF_LEFT) {
								sda = Math.abs(fonts[curfont].symbols[s
										.charAt(i)].rightx);
								sdb = Math.abs(fonts[curfont].symbols[s
										.charAt(i + 1)].leftx);
							} else {
	...
	public void glfGetStringBoundsF(int fd, String s, float[] minx,
	...
	public void glfGetStringBounds(String s, float[] minx, float[] miny,
	...
	// }
	//
	// static void latorgba(unsigned char *b, unsigned char *a,unsigned char
	// *l,int n)
	// {
	// while (n--) {
	// l[0] = *b;
	...
	// }
	//
	// static void rgbtorgba(unsigned char *r,unsigned char *g,unsigned char
	// *b,unsigned char *l,int n)
	// {
	// while (n--) {
	// l[0] = r[0];
	// l[1] = g[0];
	...
	// }
	//
	// static void rgbatorgba(unsigned char *r,unsigned char *g,unsigned char
	// *b,unsigned char *a,unsigned char *l,int n)
	// {
	// while (n--) {
	// l[0] = r[0];
	// l[1] = g[0];
	...
	// static void ImageGetRow(ImageRec *image, unsigned char *buf, int y, int
	// z)
	// {
	...
	// }
	// } else {
	// fseek(image.file, 512 + (y * image.xsize) + (z * image.xsize *
	// image.ysize) + SEEK_SET_POS, SEEK_SET);
	// fread(buf, 1, image.xsize, image.file);
	// }
	// }
	//
	// /* Read SGI (RGB) Image from file */
	// static int[] read_texture(LittleEndianInputStream in, int width, int
	// height, int components)
	// {
	// unsigned * base, *lptr;
	...
	// gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, 3, twidth, theight, 0, GL.GL_RGBA,
	// GL.GL_UNSIGNED_BYTE, texture);
	//
	...
	//
	// gl.glEndList();
	// if (Symbols[i].height > m_max_height[bmf_curfont])
	// m_max_height[bmf_curfont] = Symbols[i].height;
	// }
	// return bmf_curfont;
	// }
	//
	...
	// if ((bmf_descriptor < 0) || (bmf_in_use[bmf_descriptor] == false)) return
	// GLF_ERROR;
	//
	...
	// if ((Font_Descriptor < 0) || (bmf_in_use[Font_Descriptor] == false))
	// return GLF_ERROR;
	//
