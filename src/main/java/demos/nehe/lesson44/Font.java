...
class Font {
    ...
    private ByteBuffer stringBuffer = GLBuffers.newDirectByteBuffer(512);

    public Font() {
    	// Initalize the texture to 0
        m_FontTexture = 0;
     // Initalize the List base to 0
        m_ListBase = 0;    
    }
    ...
    public void buildFont(GL2 gl, float scale) {
        ...
        if (m_FontTexture != 0) {
            ...
            for (int loop = 0; loop < 256; loop++) {                             // Loop Through All 256 Lists
                ...
                gl.glNewList(m_ListBase + loop, GL2.GL_COMPILE);              // Start Building A List
    ...
    public void glPrintf(GL2 gl, int x, int y, int set, String format) {
        ...
        if (stringBuffer.capacity() < format.length()) {
            stringBuffer = GLBuffers.newDirectByteBuffer(format.length());
        }
