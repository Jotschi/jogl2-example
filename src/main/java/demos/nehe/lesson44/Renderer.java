...
class Renderer implements GLEventListener {
	...
	public void moveCameraBackward() {
		gCamera.m_ForwardVelocity = -.01f;// Start moving the camera backwards
											// 0.01 units every frame
	}

	public void moveCameraForward() {
		gCamera.m_ForwardVelocity = .01f; // Start moving the camera forward
											// 0.01 units every frame
	}
	...
	private void loadTexture(String fileName, int[] texid,
			GLAutoDrawable drawable) throws IOException { // Creates Texture
															// From A Bitmap
															// File
		...
		gl.glPixelStorei(GL2.GL_UNPACK_ALIGNMENT, 4); // Pixel Storage Mode
														// (Word Alignment / 4
														// Bytes)

		// Typical Texture Generation Using Data From The Bitmap
		gl.glBindTexture(GL2.GL_TEXTURE_2D, texid[0]); // Bind To The Texture ID
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
				GL2.GL_LINEAR); // Linear Min Filter
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_LINEAR); // Linear Mag Filter
		gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, 3, texture.getWidth(),
				texture.getHeight(), 0, GL2.GL_BGR, GL2.GL_UNSIGNED_BYTE,
				texture.getPixels());
	}
	...
	public void init(GLAutoDrawable drawable) {
		...
		try {
			loadTexture("demos/data/images/Font.bmp", tex, drawable); // Load
																		// the
																		// font
																		// texture
		} catch (IOException e) {
		...
		if (tex[0] != 0) { // Make sure it was loaded
			...
			gFont.setWindowSize(1024, 768); // The font class needs to know the
											// window size
		...
		// Try and load the Halo texture tell the user if we can't find it then
		// quit
		try {
			loadTexture("demos/data/images/Halo3.bmp", gCamera.m_HaloTexture,
					drawable);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}

		// Try and load the Streaks texture tell the user if we can't find it
		// then quit
		try {
	...
	public void display(GLAutoDrawable drawable) {
		...
		gCamera.setPrespective(gl); // Set our perspective/oriention on the
									// world
		gCamera.renderLensFlare(gl); // Render the lens flare
		gCamera.updateFrustumFaster(gl); // Update the frustum as fast as
											// possible.
	...
	private void drawGLInfo(GL2 gl) {

		float modelMatrix[] = new float[16], // This will hold the model view
												// matrix
		...
		gl.glGetFloatv(GLMatrixFunc.GL_PROJECTION_MATRIX, projMatrix, 0); // Grab
																			// the
																			// projection
																			// matrix
		gl.glGetFloatv(GLMatrixFunc.GL_MODELVIEW_MATRIX, modelMatrix, 0); // Grab
																			// the
																			// modelview
																			// matrix
		...
		string = "m_Position............. = "
				+ twoFracsMax(gCamera.m_Position.x) + ", "
		...
		string = "m_DirectionVector...... = "
				+ twoFracsMax(gCamera.m_DirectionVector.x) + ", "
		...
		string = "m_LightSourcePos....... = "
				+ twoFracsMax(gCamera.m_LightSourcePos.x) + ", "
		...
		string = "vLightSourceToCamera... = "
				+ twoFracsMax(gCamera.vLightSourceToCamera.x) + ", "
				+ twoFracsMax(gCamera.vLightSourceToCamera.y) + ", "
				+ twoFracsMax(gCamera.vLightSourceToCamera.z);
		gFont.glPrintf(gl, 10, 640, 1, string);

		// Print out the vector that points from the light source to the
		// intersection point.
		string = "vLightSourceToIntersect = "
				+ twoFracsMax(gCamera.vLightSourceToIntersect.x) + ", "
		...
		string = twoFracsMax(modelMatrix[0]) + ", "
				+ twoFracsMax(modelMatrix[1]) + ", "
		...
		string = twoFracsMax(modelMatrix[4]) + ", "
				+ twoFracsMax(modelMatrix[5]) + ", "
		...
		string = twoFracsMax(modelMatrix[8]) + ", "
				+ twoFracsMax(modelMatrix[9]) + ", "
		...
		string = twoFracsMax(modelMatrix[12]) + ", "
				+ twoFracsMax(modelMatrix[13]) + ", "
		...
		string = twoFracsMax(projMatrix[12]) + ", "
				+ twoFracsMax(projMatrix[13]) + ", "
		...
		if (gFrames >= 100) { // if we are due for another FPS update
			...
			DiffTime = (float) (gCurrentTime - gStartTime); // Find the
															// difference
															// between the start
															// and end times
			gFPS = (gFrames / DiffTime) * 1000.0f; // Compute the FPS
			gStartTime = gCurrentTime; // Set the current start time to the
										// current time
			gFrames = 1; // Set the number of frames to 1
		} else {
			gFrames++; // We are not due to for another update so add one to the
						// frame count
		}
	...
	public void reshape(GLAutoDrawable drawable, int xstart, int ystart,
		...
		gCamera.m_WindowHeight = height; // The camera needs to know the window
											// height
		gCamera.m_WindowWidth = width; // The camera needs to know the window
										// width
