 ...
 * give me credit for it if you do. And plug NeHe while your at it! :P Thanks go
 * to David Steere, Cameron Tidwell, Bert Sammons, and Brannon Martindale for
...
public class Camera {
	...
	private void updateFrustum(GL gl) {
		...
		gl.glGetFloatv(GLMatrixFunc.GL_MODELVIEW_MATRIX, modl, 0);
	...
	public void updateFrustumFaster(GL gl) {
		...
		/*
		 * Combine the two matrices (multiply projection by modelview) but keep
		 * in mind this function will only work if you do NOT rotate or
		 * translate your projection matrix
		 */
	...
	/**
	 * This member function checks to see if a sphere is in the viewing volume.
	 * 
	 * @param p
	 * @param Radius
	 * @return
	...
	private boolean sphereInFrustum(Tuple3f p, float Radius) {
		...
		for (i = 0; i < 6; i++) {
			// If the point is outside of the plane then its not in the viewing
			// volume.
			if (m_Frustum[i][0] * p.x + m_Frustum[i][1] * p.y + m_Frustum[i][2]
					* p.z + m_Frustum[i][3] <= -Radius) {
				return false;
			}
		}
		return true;
	...
	/**
	 * This member fuction checks to see if a point is in the viewing volume.
	 * 
	 * @param p
	 * @return
	 */
	...
	public void renderLensFlare(GL2 gl) {
		...
		if (sphereInFrustum(m_LightSourcePos, 1.0f)) {
			...
			vLightSourceToCamera.sub(m_Position, m_LightSourcePos); // Lets
																	// compute
																	// the
																	// vector
																	// that
																	...
																	// from
			// the light source.
			Length = vLightSourceToCamera.length(); // Save the length we will
													// need it in a minute
			ptIntersect.scale(Length, m_DirectionVector); // Now lets find an
															// point along the
															// cameras direction
			...
			vLightSourceToIntersect.sub(ptIntersect, m_LightSourcePos); // Lets
																		// compute
																		// the
																		// vector
																		// that
			...
			gl.glBlendFunc(GL.GL_SRC_ALPHA, GL.GL_ONE); // You should already
														// know what this does
			gl.glDisable(GL.GL_DEPTH_TEST); // You should already know what this
											// does
			gl.glEnable(GL.GL_TEXTURE_2D); // You should already know what this
											// does
			...
			pt.scaleAdd(Length * .1f, vLightSourceToIntersect, m_LightSourcePos); // Lets
																					// compute
																					// a
																					// point
																					// that
			...
			renderGlow(gl, 0.9f, 0.6f, 0.4f, 0.5f, pt, 0.6f); // Render the
																// small Glow

			pt.scaleAdd(Length * .15f, vLightSourceToIntersect,
					m_LightSourcePos); // Lets compute a point that is 30%
			// away from the light source in the
			// direction of the intersection point.
			renderHalo(gl, 0.8f, 0.5f, 0.6f, 0.5f, pt, 1.7f); // Render the a
																// Halo

			pt.scaleAdd(Length * .175f, vLightSourceToIntersect,
					m_LightSourcePos); // Lets compute a point that is 35%
			// away from the light source in the
			// direction of the intersection point.
			renderHalo(gl, 0.9f, 0.2f, 0.1f, 0.5f, pt, 0.83f); // Render the a
																// Halo

			pt.scaleAdd(Length * .285f, vLightSourceToIntersect,
					m_LightSourcePos); // Lets compute a point that is 57%
			// away from the light source in the
			// direction of the intersection point.
			renderHalo(gl, 0.7f, 0.7f, 0.4f, 0.5f, pt, 1.6f); // Render the a
																// Halo

			pt.scaleAdd(Length * .2755f, vLightSourceToIntersect,
					m_LightSourcePos);// Lets compute a point that is 55.1%
			// away from the light source in the
			// direction of the intersection point.
			renderGlow(gl, 0.9f, 0.9f, 0.2f, 0.5f, pt, 0.8f); // Render the
																// small Glow

			pt.scaleAdd(Length * .4775f, vLightSourceToIntersect,
					m_LightSourcePos);// Lets compute a point that is 95.5%
			// away from the light source in the
			// direction of the intersection point.
			renderGlow(gl, 0.93f, 0.82f, 0.73f, 0.5f, pt, 1.0f); // Render the
																	// small
																	// Glow

			pt.scaleAdd(Length * .49f, vLightSourceToIntersect,
					m_LightSourcePos); // Lets compute a point that is 98%
			// away from the light source in the
			// direction of the intersection point.
			renderHalo(gl, 0.7f, 0.6f, 0.5f, 0.5f, pt, 1.4f); // Render the a
																// Halo

			pt.scaleAdd(Length * .65f, vLightSourceToIntersect,
					m_LightSourcePos); // Lets compute a point that is 130%
			// away from the light source in the
			// direction of the intersection point.
			renderGlow(gl, 0.7f, 0.8f, 0.3f, 0.5f, pt, 1.8f); // Render the
																// small Glow

			pt.scaleAdd(Length * 0.63f, vLightSourceToIntersect,
					m_LightSourcePos); // Lets compute a point that is 126%
			// away from the light source in the
			// direction of the intersection point.
			renderGlow(gl, 0.4f, 0.3f, 0.2f, 0.5f, pt, 1.4f); // Render the
																// small Glow

			pt.scaleAdd(Length * 0.8f, vLightSourceToIntersect,
					m_LightSourcePos); // Lets compute a point that is 160%
			// away from the light source in the
			// direction of the intersection point.
			renderHalo(gl, 0.7f, 0.5f, 0.5f, 0.5f, pt, 1.4f); // Render the a
																// Halo

			pt.scaleAdd(Length * .7825f, vLightSourceToIntersect,
					m_LightSourcePos);// Lets compute a point that is 156.5%
			// away from the light source in the
			// direction of the intersection point.
			renderGlow(gl, 0.8f, 0.5f, 0.1f, 0.5f, pt, 0.6f); // Render the
																// small Glow

			pt.scaleAdd(Length * 1.0f, vLightSourceToIntersect,
					m_LightSourcePos); // Lets compute a point that is 200%
			// away from the light source in the
			// direction of the intersection point.
			renderHalo(gl, 0.5f, 0.5f, 0.7f, 0.5f, pt, 1.7f); // Render the a
																// Halo

			pt.scaleAdd(Length * 0.975f, vLightSourceToIntersect,
					m_LightSourcePos);// Lets compute a point that is 195%
			// away from the light source in the
			// direction of the intersection point.
			renderGlow(gl, 0.4f, 0.1f, 0.9f, 0.5f, pt, 2.0f); // Render the
																// small Glow

			gl.glDisable(GL.GL_BLEND); // You should already know what this does
			gl.glEnable(GL.GL_DEPTH_TEST); // You should already know what this
											// does
			gl.glDisable(GL.GL_TEXTURE_2D); // You should already know what this
											// does
		}
	}
	...
	private void renderGlow(GL2 gl, float r, float g, float b, float a,
		...
		// we are rotating the camera by the inverse so the
		// texture mapped quads will always face us.
		q[0].x = (p.x - scale); // Set the x coordinate -scale units from the
								// center point.
		q[0].y = (p.y - scale); // Set the y coordinate -scale units from the
								// center point.

		q[1].x = (p.x - scale); // Set the x coordinate -scale units from the
								// center point.
		q[1].y = (p.y + scale); // Set the y coordinate scale units from the
								// center point.

		q[2].x = (p.x + scale); // Set the x coordinate scale units from the
								// center point.
		q[2].y = (p.y - scale); // Set the y coordinate -scale units from the
								// center point.

		q[3].x = (p.x + scale); // Set the x coordinate scale units from the
								// center point.
		q[3].y = (p.y + scale); // Set the y coordinate scale units from the
								// center point.
		...
		gl.glRotatef(-m_HeadingDegrees, 0.0f, 1.0f, 0.0f);
		gl.glRotatef(-m_PitchDegrees, 1.0f, 0.0f, 0.0f);
		gl.glBindTexture(GL.GL_TEXTURE_2D, m_GlowTexture[0]); // Bind to the Big
																// Glow texture
	...
	private void renderBigGlow(GL2 gl, float r, float g, float b, float a,
		...
		gl.glRotatef(-m_HeadingDegrees, 0.0f, 1.0f, 0.0f);
		gl.glRotatef(-m_PitchDegrees, 1.0f, 0.0f, 0.0f);
		gl.glBindTexture(GL.GL_TEXTURE_2D, m_BigGlowTexture[0]); // Bind to the
																	// Big Glow
																	// texture
		gl.glColor4f(r, g, b, a); // Set the color since the texture is a gray
									// scale
	...
	private void renderStreaks(GL2 gl, float r, float g, float b, float a,
			Tuple3f p, float scale) {
