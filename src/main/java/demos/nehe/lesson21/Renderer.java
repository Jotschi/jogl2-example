...
class Renderer implements GLEventListener {
	...
	private boolean[][] vline = new boolean[11][11]; // Keeps Track Of Verticle
														// Lines
	private boolean[][] hline = new boolean[11][11]; // Keeps Track Of
														// Horizontal Lines
	...
	private int steps[] = { 1, 2, 4, 5, 10, 20 }; // Stepping Values For Slow
													// Video Adjustment
	...
	private void resetObjects() { // Reset Player And Enemies
		...
		for (int i = 0; i < (stage * level); i++) { // Loop Through All The
													// Enemies
			enemy[i] = new GameObject();
			enemy[i].x = 5 + (int) (Math.random() * 6); // Select A Random X
														// Position
	...
	void buildFont(GL2 gl) { // Build Our Font Display List
		base = gl.glGenLists(256); // Creating 256 Display Lists
		for (int i = 0; i < 256; i++) // Loop Through All 256 Lists
		{
			float cx = (float) (i % 16) / 16.0f; // X Position Of Current
													// Character
			float cy = (float) (i / 16) / 16.0f; // Y Position Of Current
													// Character

			gl.glNewList(base + i, GL2.GL_COMPILE); // Start Building A List
			...
			gl.glTexCoord2f(cx + 0.0625f, 1.0f - cy); // Texture Coord (Top
														// Right)
	...
	private void glPrint(GL2 gl, int x, int y, int set, String message) // Where
																		// The
																		// Printing
																		// Happens
	{
		...
		if (stringBuffer.capacity() < message.length()) {
			stringBuffer = GLBuffers.newDirectByteBuffer(message.length());
		}

		stringBuffer.clear();
		stringBuffer.put(message.getBytes());
		stringBuffer.flip();
		gl.glCallLists(message.length(), GL.GL_UNSIGNED_BYTE, stringBuffer); // Write
																				// The
																				// Text
																				// To
																				// The
	...
	public void init(GLAutoDrawable drawable) {
		...
		gl.glBlendFunc(GL2.GL_SRC_ALPHA, GL2.GL_ONE_MINUS_SRC_ALPHA); // Type Of
																		// Blending
																		// To
																		// Use
		...
		try {
			dieSample = new AudioSample(
					ResourceRetriever
							.getResourceAsStream("demos/data/samples/Die.wav"));
			completerSample = new AudioSample(
					ResourceRetriever
							.getResourceAsStream("demos/data/samples/Complete.wav"));
			freezeSample = new AudioSample(
					ResourceRetriever
							.getResourceAsStream("demos/data/samples/Freeze.wav"));
			hourglassSample = new AudioSample(
					ResourceRetriever
							.getResourceAsStream("demos/data/samples/Hourglass.wav"));
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}
	...
	private void update() {
		...
		{
			for (int loop1 = 0; loop1 < (stage * level); loop1++) // Loop
																	// Through
																	// The
																	// Different
																	// Stages
			{
				...
				if (delay > (3 - level) && (hourglass.fx != 2)) // If Our Delay
																// Is Done And
																// Player
																// Doesn't Have
																// Hourglass
				{
					...
					for (int loop2 = 0; loop2 < (stage * level); loop2++) // Loop
																			// Through
																			// All
																			// The
																			// Enemies
					{
						if (enemy[loop2].fx < enemy[loop2].x * 60) // Is Fine
																	// Position
																	// On X Axis
																	// Lower
																	// Than
																	...
																	// Position?
						{
							enemy[loop2].fx += steps[adjust]; // If So, Increase
																// Fine Position
																// On X Axis
							enemy[loop2].spin += steps[adjust]; // Spin Enemy
																// Clockwise
						}
						if (enemy[loop2].fx > enemy[loop2].x * 60) // Is Fine
																	// Position
																	...
																	// Position?
						{
							enemy[loop2].fx -= steps[adjust]; // If So, Decrease
																// Fine Position
																// On X Axis
							enemy[loop2].spin -= steps[adjust]; // Spin Enemy
																// Counter
						...
						if (enemy[loop2].fy < enemy[loop2].y * 40) // Is Fine
																	// Position
																	// On Y Axis
																	// Lower
																	// Than
																	...
																	// Position?
						{
							enemy[loop2].fy += steps[adjust]; // If So, Increase
																// Fine Position
																// On Y Axis
							enemy[loop2].spin += steps[adjust]; // Spin Enemy
																// Clockwise
						...
						if (enemy[loop2].fy > enemy[loop2].y * 40) // Is Fine
																	...
																	// Position?
						{
							enemy[loop2].fy -= steps[adjust]; // If So, Decrease
																// Fine Position
																// On Y Axis
							enemy[loop2].spin -= steps[adjust]; // Spin Enemy
																// Counter
			...
			if (moveRight && (player.x < 10) && (player.fx == player.x * 60)
					&& (player.fy == player.y * 40)) {
				hline[player.x][player.y] = true; // Mark The Current Horizontal
													// Border As Filled
				player.x++; // Move The Player Right
			}
			if (moveLeft && (player.x > 0) && (player.fx == player.x * 60)
					&& (player.fy == player.y * 40)) {
				player.x--; // Move The Player Left
				hline[player.x][player.y] = true; // Mark The Current Horizontal
													// Border As Filled
			}
			if (moveDown && (player.y < 10) && (player.fx == player.x * 60)
					&& (player.fy == player.y * 40)) {
				vline[player.x][player.y] = true; // Mark The Current Verticle
													// Border As Filled
				player.y++; // Move The Player Down
			}
			if (moveUp && (player.y > 0) && (player.fx == player.x * 60)
					&& (player.fy == player.y * 40)) {
			...
			if (player.fx < player.x * 60) // Is Fine Position On X Axis Lower
											// Than Intended Position?
			{
				player.fx += steps[adjust]; // If So, Increase The Fine X
											// Position
			}
			if (player.fx > player.x * 60) // Is Fine Position On X Axis Greater
											// Than Intended Position?
			{
				player.fx -= steps[adjust]; // If So, Decrease The Fine X
											// Position
			}
			if (player.fy < player.y * 40) // Is Fine Position On Y Axis Lower
											// Than Intended Position?
			{
				player.fy += steps[adjust]; // If So, Increase The Fine Y
											// Position
			}
			if (player.fy > player.y * 40) // Is Fine Position On Y Axis Lower
											// Than Intended Position?
			{
				player.fy -= steps[adjust]; // If So, Decrease The Fine Y
											// Position
			}
		} else // Otherwise
		{
		...
		{
			completerSample.play(true, false); // If So, Play The Level Complete
												// Sound
			...
			for (int loop1 = 0; loop1 < 11; loop1++) // Loop Through The Grid X
														// Coordinates
			{
				for (int loop2 = 0; loop2 < 11; loop2++) // Loop Through The
															// Grid Y
															// Coordinates
				{
					if (loop1 < 10) // If X Coordinate Is Less Than 10
					{
						hline[loop1][loop2] = false; // Set The Current
														// Horizontal Value To
														// FALSE
					}
					if (loop2 < 10) // If Y Coordinate Is Less Than 10
					{
						vline[loop1][loop2] = false; // Set The Current Vertical
														// Value To FALSE
					}
				}
			}
		...
		// If The Player Hits The Hourglass While It's Being Displayed On The
		// Screen
		...
		hourglass.spin -= 0.25f * steps[adjust]; // Spin The Hourglass Counter
													// Clockwise
		...
		{ // Variable Greater Than 6000 Divided By The Current Level?
			hourglassSample.play(false, false); // If So, Play The Hourglass
												// Appears Sound
			hourglass.x = Math.abs(random.nextInt()) % 10 + 1; // Give The
																// Hourglass A
																// Random X
																// Value
			hourglass.y = Math.abs(random.nextInt()) % 11; // Give The Hourglass
															// A Random Y Value
			hourglass.fx = 1; // Set hourglass fx Variable To One (Hourglass
								// Stage)
		...
		if ((hourglass.fx == 2) && (hourglass.fy > 500 + (500 * level))) // Is
																			// The
																			// hourglass
																			// fx
																			// Variable
	...
	public void drawGLScene(GL2 gl) {
		...
		gl.glBindTexture(GL.GL_TEXTURE_2D, textures[0]); // Select Our Font
															// Texture
		...
		if (gameover) { // Is The Game Over?
			...
			glPrint(gl, 472, 20, 1, "GAME OVER"); // Write GAME OVER To The
													// Screen
			glPrint(gl, 456, 40, 1, "PRESS SPACE"); // Write PRESS SPACE To The
													// Screen
		}

		for (int loop1 = 0; loop1 < lives - 1; loop1++) { // Loop Through Lives
															// Minus Current
															// Life
			gl.glLoadIdentity(); // Reset The View
			gl.glTranslatef(490 + (loop1 * 40.0f), 40.0f, 0.0f); // Move To The
																	// Right Of
																	// Our Title
																	// Text
			gl.glRotatef(-player.spin, 0.0f, 0.0f, 1.0f); // Rotate Counter
															// Clockwise
			...
			gl.glRotatef(-player.spin * 0.5f, 0.0f, 0.0f, 1.0f); // Rotate
																	// Counter
																	// Clockwise
		...
		for (int loop1 = 0; loop1 < 11; loop1++) { // Loop From Left To Right
			for (int loop2 = 0; loop2 < 11; loop2++) { // Loop From Top To
														// Bottom
				gl.glColor3f(0.0f, 0.5f, 1.0f); // Set Line Color To Blue
				if (hline[loop1][loop2]) { // Has The Horizontal Line Been
											// Traced
					gl.glColor3f(1.0f, 1.0f, 1.0f); // If So, Set Line Color To
													// White
				}

				if (loop1 < 10) { // Dont Draw To Far Right
					if (!hline[loop1][loop2]) { // If A Horizontal Line Isn't
												// Filled
						filled = false; // filled Becomes False
					}
					gl.glBegin(GL.GL_LINES); // Start Drawing Horizontal Cell
												// Borders
					gl.glVertex2d(20 + (loop1 * 60), 70 + (loop2 * 40));// Left
																		// Side
																		// Of
																		// Horizontal
																		// Line
					gl.glVertex2d(80 + (loop1 * 60), 70 + (loop2 * 40));// Right
																		// Side
																		// Of
																		// Horizontal
																		// Line
				...
				gl.glColor3f(0.0f, 0.5f, 1.0f); // Set Line Color To Blue
				if (vline[loop1][loop2]) { // Has The Horizontal Line Been
											// Traced
					gl.glColor3f(1.0f, 1.0f, 1.0f); // If So, Set Line Color To
													// White
				}
				if (loop2 < 10) { // Dont Draw To Far Down
					if (!vline[loop1][loop2]) { // If A Verticle Line Isn't
												// Filled
						filled = false; // filled Becomes False
					}
					gl.glBegin(GL.GL_LINES); // Start Drawing Verticle Cell
												// Borders
					gl.glVertex2d(20 + (loop1 * 60), 70 + (loop2 * 40));// Left
																		// Side
																		// Of
																		// Horizontal
																		// Line
					gl.glVertex2d(20 + (loop1 * 60), 110 + (loop2 * 40));// Right
																			// Side
																			// Of
																			// Horizontal
																			// Line
				...
				gl.glBindTexture(GL.GL_TEXTURE_2D, textures[1]); // Select The
																	// Tile
																	// Image
				if ((loop1 < 10) && (loop2 < 10)) { // If In Bounds, Fill In
													// Traced Boxes
					// Are All Sides Of The Box Traced?
					if (hline[loop1][loop2] && hline[loop1][loop2 + 1]
							&& vline[loop1][loop2] && vline[loop1 + 1][loop2]) {
						gl.glBegin(GL2.GL_QUADS); // Draw A Textured Quad
						gl.glTexCoord2f((loop1 / 10.0f) + 0.1f,
								1.0f - ((loop2 / 10.0f)));
						gl.glVertex2d(20 + (loop1 * 60) + 59,
								(70 + loop2 * 40 + 1)); // Top Right
						gl.glTexCoord2f((loop1 / 10.0f),
								1.0f - ((loop2 / 10.0f)));
						gl.glVertex2d(20 + (loop1 * 60) + 1,
								(70 + loop2 * 40 + 1)); // Top Left
						gl.glTexCoord2f((loop1 / 10.0f),
								1.0f - ((loop2 / 10.0f) + 0.1f));
						gl.glVertex2d(20 + (loop1 * 60) + 1,
		...
		if (hourglass.fx == 1) { // If fx=1 Draw The Hourglass
			gl.glLoadIdentity(); // Reset The Modelview Matrix
			gl.glTranslatef(20.0f + (hourglass.x * 60),
					70.0f + (hourglass.y * 40), 0.0f); // Move To The Fine
														// Hourglass Position
			gl.glRotatef(hourglass.spin, 0.0f, 0.0f, 1.0f); // Rotate Clockwise
			gl.glColor3f(random.nextFloat(), random.nextFloat(),
					random.nextFloat()); // Set Hourglass Color To Random Color
		...
		gl.glLoadIdentity(); // Reset The Modelview Matrix
		gl.glTranslatef(player.fx + 20.0f, player.fy + 70.0f, 0.0f); // Move To
																		// The
																		// Fine
																		// Player
																		// Position
		...
		for (int loop1 = 0; loop1 < (stage * level); loop1++) { // Loop To Draw
																// Enemies
			gl.glLoadIdentity(); // Reset The Modelview Matrix
			gl.glTranslatef(enemy[loop1].fx + 20.0f, enemy[loop1].fy + 70.0f,
					0.0f);
			...
			gl.glRotatef(enemy[loop1].spin, 0.0f, 0.0f, 1.0f); // Rotate The
																// Enemy Blade
	...
	public void reshape(GLAutoDrawable drawable, int xstart, int ystart,
		...
		gl.glLoadIdentity();
		gl.glOrtho(0.0f, width, height, 0.0f, -1.0f, 1.0f); // Create Ortho
															// 640x480 View (0,0
															// At Top Left)
