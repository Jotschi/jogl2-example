...
class Renderer implements GLEventListener {
	...
	public void display(GLAutoDrawable glDrawable) {
		...
		gl.glBindTexture(GL.GL_TEXTURE_2D, textures[0]); // Select Our Logo
															// Texture
		gl.glBegin(GL2.GL_QUADS); // Start Drawing A Textured Quad
		...
		{
			gl.glBlendFunc(GL.GL_DST_COLOR, GL.GL_ZERO); // Blend Screen Color
															// With Zero (Black)
		}

		if (scene) // Are We Drawing The Second Scene?
		{
			gl.glTranslatef(0.0f, 0.0f, -1.0f); // Translate Into The Screen One
												// Unit
			gl.glRotatef(roll * 360, 0.0f, 0.0f, 1.0f); // Rotate On The Z Axis
														// 360 Degrees.
			if (maskingEnabled) // Is Masking On?
			{
				gl.glBindTexture(GL.GL_TEXTURE_2D, textures[3]); // Select The
																	// Second
																	// Mask
																	// Texture
				gl.glBegin(GL2.GL_QUADS); // Start Drawing A Textured Quad
			...
			gl.glBlendFunc(GL.GL_ONE, GL.GL_ONE); // Copy Image 2 Color To The
													// Screen
			gl.glBindTexture(GL.GL_TEXTURE_2D, textures[4]); // Select The
																// Second Image
																// Texture
			gl.glBegin(GL2.GL_QUADS); // Start Drawing A Textured Quad
			gl.glTexCoord2f(0.0f, 0.0f);
		...
		} else // Otherwise
		{
			...
			gl.glBlendFunc(GL.GL_ONE, GL.GL_ONE); // Copy Image 1 Color To The
													// Screen
			gl.glBindTexture(GL.GL_TEXTURE_2D, textures[2]); // Select The First
																// Image Texture
			gl.glBegin(GL2.GL_QUADS); // Start Drawing A Textured Quad
			gl.glTexCoord2f(roll + 0.0f, 0.0f);
			gl.glVertex3f(-1.1f, -1.1f, 0.0f); // Bottom Left
