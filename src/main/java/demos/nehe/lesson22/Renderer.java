...
class Renderer implements GLEventListener {
	...
	private boolean multitextureEnabled = true; 
	// Used To Disable ARB Extensions entirely
	private boolean __ARB_ENABLE = true; 
	// Emboss Only, No Basetexture?
	private boolean embossEnabled = false; 
	// Do Bumpmapping?
	private boolean bumpsEnabled = true; 

	// Number Of Texel-Pipelines. This Is At Least 1.
	private int[] maxTexelUnits = new int[1];
	// Handle For Multitexture-Enabled-Logo
	private int[] multiLogo = new int[1];
	// Inverted Bumpmap
	private int[] invbump = new int[3]; 
	// Storage For 3 Textures
	private int[] textures = new int[3];
	// Handle For OpenGL-Logo
	private int[] glLogo = new int[1];
	// Which Filter To Use
	private int filter = 1; 
	// Our Bumpmappings
	private int[] bump = new int[3]; 
	...
	// Maximum Emboss-Translate. Increase To Get Higher Immersion At A Cost Of Lower Quality (More Artifacts Will Occur!)
	private float MAX_EMBOSS = 8e-3f; 
	// Gray Color
	private float[] gray = Color.gray.getRGBComponents(null); 
	...
	// Supported
	private boolean initMultitexture(GL2 gl) {

		String extensions;
		extensions = gl.glGetString(GL2.GL_EXTENSIONS); // Fetch Extension
														// String

		int multiTextureAvailable = extensions.indexOf("GL_ARB_multitexture");
		int textureEnvCombineAvailable = extensions
				.indexOf("GL_ARB_texture_env_combine");
		if (multiTextureAvailable != -1 // Is Multitexturing Supported?
				&& __ARB_ENABLE // Override-Flag
				&& textureEnvCombineAvailable != -1) {// Is
	 ...
	 * @throws IOException
	 */
	private void loadGLTextures(GL2 gl, GLUgl2 glu) throws IOException { 
		TextureReader.Texture texture = TextureReader
				.readTexture("demos/data/images/Base.bmp");
		...
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
				GL2.GL_NEAREST);
		gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, GL2.GL_RGB8, texture.getWidth(),
		...
		glu.gluBuild2DMipmaps(GL2.GL_TEXTURE_2D, GL2.GL_RGB8,
				texture.getWidth(), texture.getHeight(), GL2.GL_RGB,
		...
		// Half Intenstity
		gl.glPixelTransferf(GL2.GL_GREEN_SCALE, 0.5f); 
		gl.glPixelTransferf(GL2.GL_BLUE_SCALE, 0.5f);
		...
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_WRAP_T,
				GL2.GL_CLAMP);
		gl.glTexParameterfv(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_BORDER_COLOR,
				gray, 0);

		gl.glGenTextures(3, bump, 0); // Create Three Textures

		gl.glBindTexture(GL2.GL_TEXTURE_2D, bump[0]); // Create Nearest Filtered
														// Texture
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_NEAREST);
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, bump[1]); // Create Linear Filtered
														// Texture
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_LINEAR);
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, bump[2]); // Create MipMapped
														// Texture
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_LINEAR);
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, invbump[0]); // Create Nearest
															// Filtered Texture
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_NEAREST);
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, invbump[1]); // Create Linear
															// Filtered Texture
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_LINEAR);
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, invbump[2]); // Create MipMapped
															// Texture
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_LINEAR);
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
		...
		gl.glPixelTransferf(GL2.GL_RED_SCALE, 1.0f); // Scale RGB Back To 100%
														// Again
		gl.glPixelTransferf(GL2.GL_GREEN_SCALE, 1.0f);
		gl.glPixelTransferf(GL2.GL_BLUE_SCALE, 1.0f);

		texture = TextureReader.readTexture(
				"demos/data/images/OpenGL_Alpha.bmp", true); // Load The
																// Logo-Pngs
		ByteBuffer alpha = GLBuffers.newDirectByteBuffer(texture.getPixels()
				.limit()); // Create Memory For RGBA8-Texture

		for (int a = 0; a < alpha.capacity(); a += 4)
			alpha.put(a + 3, texture.getPixels().get(a)); // Pick Only Red Value
															// As Alpha!

		texture = TextureReader.readTexture("demos/data/images/OpenGL2.bmp",
				true);
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, glLogo[0]); // Create Linear
														// Filtered
														// RGBA8-Texture
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_LINEAR);
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MIN_FILTER,
				GL2.GL_LINEAR);
		gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, GL2.GL_RGBA8, texture.getWidth(),
				texture.getHeight(), 0, GL2.GL_RGBA, GL2.GL_UNSIGNED_BYTE,
				alpha);

		texture = TextureReader.readTexture(
				"demos/data/images/Multi_On_Alpha.bmp", true);// Load The
																// "Extension Enabled"-Logo
		// Create memory for RGBA8-Texture
		alpha = GLBuffers.newDirectByteBuffer(texture.getPixels().limit()); 

		for (int a = 0; a < alpha.capacity(); a += 4)
			alpha.put(a + 3, texture.getPixels().get(a)); // Pick Only Red Value
															// As Alpha!

		texture = TextureReader.readTexture("demos/data/images/Multi_On.bmp",
				true);
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, multiLogo[0]); // Create Linear
															// Filtered
															// RGBA8-Texture
		gl.glTexParameteri(GL2.GL_TEXTURE_2D, GL2.GL_TEXTURE_MAG_FILTER,
				GL2.GL_LINEAR);
	...
	void doLogo(GL2 gl) {// MUST CALL THIS LAST!!!, Billboards The Two Logos.
		...
		gl.glLoadIdentity();
		gl.glBindTexture(GL2.GL_TEXTURE_2D, glLogo[0]);
		...
		if (multitextureEnabled) {
			gl.glBindTexture(GL2.GL_TEXTURE_2D, multiLogo[0]);
			gl.glBegin(GL2.GL_QUADS);
			gl.glTexCoord2f(0.0f, 0.0f);
			gl.glVertex3f(-0.53f, -0.4f, -1.0f);
			gl.glTexCoord2f(1.0f, 0.0f);
	...
	void doMesh1TexelUnits(GL2 gl) {
		...
		gl.glGetFloatv(GLMatrixFunc.GL_MODELVIEW_MATRIX, Minv, 0);
		gl.glLoadIdentity();
		gl.glTranslatef(0.0f, 0.0f, z);
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, bump[filter]);
		...
		gl.glBindTexture(GL2.GL_TEXTURE_2D, invbump[filter]);
	...
	void doMesh2TexelUnits(GL2 gl) {
		...
		for (i = 0; i < 4; i++) {
			...
			c[2] = data[5 * i + 4];
			SetUpBumps(n, c, l, s, t);
			gl.glMultiTexCoord2f(GL2.GL_TEXTURE0, data[5 * i], data[5 * i + 1]);
			gl.glMultiTexCoord2f(GL2.GL_TEXTURE1, data[5 * i] + c[0],
					data[5 * i + 1] + c[1]);
			gl.glVertex3f(data[5 * i + 2], data[5 * i + 3], data[5 * i + 4]);
		}
	...
	private void doMeshNoBumps(GL2 gl) {
