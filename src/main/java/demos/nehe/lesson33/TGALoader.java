...
class TGALoader {
    ...
    static {
        ...
        uTGAcompare = GLBuffers.newDirectByteBuffer(uncompressedTgaHeader.length);
        uTGAcompare.put(uncompressedTgaHeader);	// Uncompressed TGA Header
        uTGAcompare.flip();

        cTGAcompare = GLBuffers.newDirectByteBuffer(compressedTgaHeader.length);
    ...
    {
        ByteBuffer header = GLBuffers.newDirectByteBuffer(12);
    ...
    {																			// TGA Loading code nehe.gamedev.net)
        ...
        texture.imageData = GLBuffers.newDirectByteBuffer(tga.imageSize);					// Allocate that much memory
    ...
    private static void loadCompressedTGA(Texture texture, ReadableByteChannel fTGA) throws IOException		// Load COMPRESSED TGAs
    {
        ...
        ByteBuffer colorbuffer = GLBuffers.newDirectByteBuffer(tga.bytesPerPixel);			// Storage for 1 pixel
