...
class Renderer implements GLEventListener {
	...
	private void setupWorld() throws IOException {
		...
		try {
			in = new BufferedReader(
					new InputStreamReader(ResourceRetriever
							.getResourceAsStream("demos/data/models/World.txt")));
			String line = null;
			while ((line = in.readLine()) != null) {
				...
				if (line.startsWith("NUMPOLLIES")) {
					...
					numTriangles = Integer
							.parseInt(line.substring(line.indexOf("NUMPOLLIES")
									+ "NUMPOLLIES".length() + 1));
			...
			for (int i = 0; i < sector1.numtriangles; i++) {
				for (int vert = 0; vert < 3; vert++) {
					...
					if (line != null) {
						...
						sector1.triangles[i].vertex[vert].x = Float.valueOf(
								st.nextToken()).floatValue();
						sector1.triangles[i].vertex[vert].y = Float.valueOf(
								st.nextToken()).floatValue();
						sector1.triangles[i].vertex[vert].z = Float.valueOf(
								st.nextToken()).floatValue();
						sector1.triangles[i].vertex[vert].u = Float.valueOf(
								st.nextToken()).floatValue();
						sector1.triangles[i].vertex[vert].v = Float.valueOf(
								st.nextToken()).floatValue();
					}
				}
			}
	...
	private void loadGLTextures(GL gl, GLUgl2 glu) throws IOException {
		TextureReader.Texture texture = TextureReader
				.readTexture("demos/data/images/mud.png");
