...
class Renderer implements GLEventListener {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        FloatBuffer shaderData = GLBuffers.newDirectFloatBuffer(96);                        // Storate For The 96 Shader Values ( NEW )

        // Start Of User Initialization
        gl.glHint(GL2.GL_PERSPECTIVE_CORRECTION_HINT, GL2.GL_NICEST); // Realy Nice perspective calculations
        ...
        gl.glEnable(GL2.GL_CULL_FACE);                               // Enable OpenGL Face Culling ( NEW )
        ...
        gl.glBindTexture(GL2.GL_TEXTURE_1D, shaderTexture[0]);         // Bind This Texture. From Now On It Will Be 1D ( NEW )
        ...
        gl.glTexImage1D(GL2.GL_TEXTURE_1D, 0, GL2.GL_RGB, 32, 0, GL2.GL_RGB, GL2.GL_FLOAT, shaderData); // Upload ( NEW )
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        gl.glGetFloatv(GLMatrixFunc.GL_MODELVIEW_MATRIX, TmpMatrix.Data, 0);     // Get The Generated Matrix ( NEW )

        // Cel-Shading Code //
        gl.glEnable(GL2.GL_TEXTURE_1D);                              // Enable 1D Texturing ( NEW )
        gl.glBindTexture(GL2.GL_TEXTURE_1D, shaderTexture[0]);       // Bind Our Texture ( NEW )
        gl.glColor3f(1.0f, 1.0f, 1.0f);                             // Set The Color Of The Model ( NEW )

        gl.glBegin(GL2.GL_TRIANGLES);                                // Tell OpenGL That We're Drawing Triangles
        ...
        if (outlineDraw) {                                            // Check To See If We Want To Draw The Outline ( NEW )
            ...
            gl.glBegin(GL2.GL_TRIANGLES);                              // Tell OpenGL What We Want To Draw
            ...
            gl.glPolygonMode(GL2.GL_BACK, GL2.GL_FILL);                  // Reset Back-Facing Polygon Drawing Mode ( NEW )
